...
/// Returns the first occurence of the longest palindrome based on the lps
/// array.
///
/// Time Complexity: O(n)
...
/// Returns a pair containing the size of the longest palindrome and the first
/// occurence of it.
///
/// Time Complexity: O(n)
pair<int, string> manacher(string &s) {
  ...
  int len = (2 * n) + 1;
  // the i-th index contains the longest palindromic substring with the i-th
  // char as the center
  vector<int> lps(len);
  int c = 0; // stores the center of the longest palindromic substring until now
  int r = 0; // stores the right boundary of the longest palindromic substring
             // until now
  int max_len = 0;
  for (int i = 0; i < len; i++) {
    ...
    // longest palindrome at center c if it is, then take r - i as lps[i] else
    // take lps[mirror] as lps[i]
    if (i < r)
    ...
    // boundary of current longest palindrome at center c if it is, the new
    // center is i
    if (i + lps[i] > r) {
      c = i;
      r = i + lps[i];
