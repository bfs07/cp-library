...
private:
  ...
  // Time Complexity: O(V)
  void build(int n, int indexed_from, vector<int> &nxt,
...
public:
  Functional_Graph(int n, int indexed_from, vector<int> &nxt,
                   vector<int> &in_degree) {
  ...
  // id of the cycle that the vertex belongs. -1 if it doesn't belong to any
  // cycle.
  vector<int> cycle_id;
  // Represents the id of the cycle of the i-th vertex. -1 if it doesn't belong
  // to any cycle.
...
private:
  ...
  void build(int n, int indexed_from, vector<int> &degree,
             vector<vector<int>> &adj) {
    queue<int> q;
...
public:
  Functional_Graph(int n, int indexed_from, vector<int> degree,
                   vector<vector<int>> &adj) {
    this->allocate(n, indexed_from);
