struct Seg_Tree {
  struct Node {
    ...
    Node() {}
    Node(int pref, int suf, int tot, int best)
        : pref(pref), suf(suf), tot(tot), best(best) {}
  };
  ...
  Seg_Tree(vi &arr) : n(arr.size()), arr(arr) {
    tree.resize(4 * n);
    build(0, n - 1, 0);
  }

  Node query(const int l, const int r, const int i, const int j,
             const int pos) {
    if (l > r || l > j || r < i)
    ...
    int mid = (l + r) / 2;
    Node left = query(l, mid, i, j, 2 * pos + 1),
         right = query(mid + 1, r, i, j, 2 * pos + 2);
    Node x;
  ...
  void update(int l, int r, const int idx, const int v, const int pos) {
    ...
    int mid = (l + r) / 2;
    update(l, mid, idx, v, 2 * pos + 1);
    update(mid + 1, r, idx, v, 2 * pos + 2);
    l = 2 * pos + 1, r = 2 * pos + 2;
    tree[pos].pref =
        max({tree[l].pref, tree[l].tot, tree[l].tot + tree[r].pref});
    tree[pos].suf = max({tree[r].suf, tree[r].tot, tree[r].tot + tree[l].suf});
    tree[pos].tot = tree[l].tot + tree[r].tot;
    tree[pos].best =
        max({tree[l].best, tree[r].best, tree[l].suf + tree[r].pref});
  }
