...
namespace graph {
  ...
  namespace detail {
    pair<bool, pair<int, int>> check_both_directed(const vector<vector<edge>> &adj, const vector<int> &in_degree) {
    ...
    void build(const int u, vector<int> &tour, vector<vector<edge>> &adj, vector<bool> &used) {
      while(!adj[u].empty()) {
  ...
  /// All vertices v should have in_degree[v] == out_degree[v]. It must not contain a specific
  /// start and end vertices.
  ///
  /// Time complexity: O(V * (log V) + E)
  bool has_euler_tour_directed(const vector<vector<edge>> &adj, const vector<int> &in_degree) {
    const pair<bool, pair<int, int>> aux = detail::check_both_directed(adj, in_degree);
  ...
  /// If there's a valid path it also returns two numbers: the source and the destination.
  /// If the source and destination can be an arbitrary vertex it will return the pair (-1, -1) 
  /// for the source and destination (it means the contains an eulerian tour).
  ///
  /// Time complexity: O(V + E)
  pair<bool, pair<int, int>> has_euler_path_directed(const vector<vector<edge>> &adj, const vector<int> &in_degree) {
    return detail::check_both_directed(adj, in_degree);
  }
 
  /// Returns the euler path. If the graph doesn't have an euler path it returns an empty vector. 
  /// 
  /// Time Complexity: O(V + E) for directed, O(V * log(V) + E) for undirected.
  /// Time Complexity: O(adj.size() + sum(adj[i].size()))
  vector<int> get_euler_path_directed(const int E, vector<vector<edge>> &adj, const vector<int> &in_degree) {
    const pair<bool, pair<int, int>> aux = has_euler_path_directed(adj, in_degree);
    const bool valid = aux.first;
    const int src = aux.second.first;
    const int dest = aux.second.second;
  ...
  /// Returns the euler tour. If the graph doesn't have an euler tour it returns an empty vector. 
  /// 
  /// Time Complexity: O(V + E)
  /// Time Complexity: O(adj.size() + sum(adj[i].size()))
  vector<int> get_euler_tour_directed(const int E, vector<vector<edge>> &adj, const vector<int> &in_degree) {
  ...
  // 
  // Time Complexity: O(V)
  pair<bool, pair<int, int>> has_euler_path_undirected(const vector<int> &degree) {
  ...
  vector<int> get_euler_tour_undirected(const int E, const vector<int> &degree, vector<vector<edge>> &adj) {
  ...
  /// Time Complexity: O(V + E)
  /// Time Complexity: O(adj.size() + sum(adj[i].size()))
  vector<int> get_euler_path_undirected(const int E, const vector<int> &degree, vector<vector<edge>> &adj) {
